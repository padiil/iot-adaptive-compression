...
func StartControlServer() {
	...
	http.HandleFunc("/case/1", func(w http.ResponseWriter, r *http.Request) {
		...
		msg := "✅ KASUS 1 (RAW): Data Santai, Net Ngebut."
		fmt.Println("\n" + msg)
		w.Write([]byte(msg))
	})
	...
	http.HandleFunc("/case/2", func(w http.ResponseWriter, r *http.Request) {
		...
		msg := "⚠️ KASUS 2 (Smooth LZ4): Sensor 40ms, Net 50ms"
		fmt.Println("\n" + msg)
		w.Write([]byte(msg))
	})
	...
	http.HandleFunc("/case/3", func(w http.ResponseWriter, r *http.Request) {
		...
		msg := "⚠️ KASUS 3 (Stable LZ4): Sensor 100ms, Net 120ms"
		fmt.Println("\n" + msg)
		w.Write([]byte(msg))
	})
	...
	http.HandleFunc("/case/4", func(w http.ResponseWriter, r *http.Request) {
		...
		msg := "🔥 KASUS 4 (GZIP): BANJIR DATA & MACET TOTAL!"
		fmt.Println("\n" + msg)
		w.Write([]byte(msg))
	})
